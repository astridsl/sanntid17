...
//Type meldinger man vil sende
//Ligger i config så kan slette denne og importere config i andersnetwork
type Msg struct {
	SomeInteger	int
	SomeArray  	[]bool
	Receiver 	string
...
func UdpRecvMsg(msgs chan<- Msg){
	...
	for {
		...
		if localIP != fromAddress.IP.String() {
			var m Msg
			Unmarshal(buf[0:n], &m) //Konvterter meldingen til den typen man vil ha
